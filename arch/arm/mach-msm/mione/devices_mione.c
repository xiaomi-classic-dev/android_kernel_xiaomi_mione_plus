...
void __init mione_add_persistent_ram(void)
{
    struct persistent_ram *pram = &mione_persistent_ram;
    struct membank* bank = &meminfo.bank[0];

    pram->start = bank->start + bank->size - MIONE_PERSISTENT_RAM_SIZE;
...
{
    mione_add_persistent_ram();
}

#ifdef CONFIG_ANDROID_RAM_CONSOLE
static char bootreason[128] = {0,};
int __init mione_boot_reason(char *s)
{
	int n;
