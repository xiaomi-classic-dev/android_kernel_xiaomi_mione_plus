...
int is_abnormal_powerup(void)
{
	...
	return pu_reason &
		(RESTART_EVENT_KPANIC | RESTART_EVENT_WDOG | RESTART_EVENT_OTHER);
}
				   ...
				   struct kobj_attribute *attr, char *buf)
{
	...
	if (reset_reason_index < RS_REASON_MAX && reset_reason_index >= 0) {
		...
		printk(KERN_DEBUG "%s: rs_reason [0x%x], first non-zero bit %d\n",
	...
	if (pu_reason_index < PU_REASON_MAX && pu_reason_index >= 0) {
		...
		printk(KERN_DEBUG "%s: pu_reason [0x%x], first non-zero bit %d\n",
